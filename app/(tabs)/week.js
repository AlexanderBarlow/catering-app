...
function sumItems(order) {
    ...
    let total = 0;
    for (const it of items) total += Number(it.qty ?? it.quantity ?? it.count ?? 1) || 1;
    return total;
}
...
function formatRange(from, to) {
    ...
    if (!Number.isFinite(a.getTime()) || !Number.isFinite(b.getTime())) return `${from} → ${to}`;
...
export default function Week() {
    ...
    const selectedLabel = useMemo(() => {
        ...
        return Number.isFinite(d.getTime()) ? d.toLocaleDateString([], { weekday: "long", month: "short", day: "numeric" }) : selectedKey;
    }, [selectedKey]);
            ...
            <View style={{ paddingHorizontal: 14, paddingBottom: 12 }}>
                ...
                >
                    <View style={{ flexDirection: "row", alignItems: "center", justifyContent: "space-between", gap: 12 }}>
                        <View style={{ flex: 1 }}>
                            <Text style={{ fontSize: 22, fontWeight: "900", color: INK }}>Week</Text>
